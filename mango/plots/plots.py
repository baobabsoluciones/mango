...
def join_html(htmls_path: list[str], button_list=None) -> BeautifulSoup:
    """
    The join_html function takes a list of html files and joins them into one html file. In the new html file, the user can
    switch between the different plots by clicking on the buttons. The function takes as input arguments:

    Args:
        htmls_path (list of str): A list of paths to the html files that will be joined.
        button_list (list of str): A list of strings that will be used as the button names.
    ...
    if all(file.endswith(".html") for file in htmls_path):
        htmls = [BeautifulSoup(open(file), "html.parser") for file in htmls_path]
    else:
        raise ValueError(
