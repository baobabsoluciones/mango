...
def plotly_plot_lines(
    ...
    """
    The plot_lines evaluator creates a plotly line chart from the given dataframe.
...
def plotly_plot_scatter(
    ...
    """
    The plot_scatter evaluator creates a scatter plot of the dataframe.
    It takes in a dataframe, x-axis column name, y-axis column name(s), and an optional color_by argument.
    The evaluator will create a scatter plot with the x-axis as the inputted x_column and each y_column on
    the y axis.
    ...
    :param Union[list, str] y_axis: Allow the evaluator to be used for both single and multiple y-axes
...
def join_html(htmls_path: list, button_list: list = None) -> BeautifulSoup:
    """
    The join_html evaluator takes a list of html files and joins them into one html file. In the new html file,
    the user can switch between the different plots by clicking on the buttons. The evaluator takes as input
    ...
    # Create the evaluator inside the script tag
    script = """
            <script>evaluator showDiv(divId) {
    ...
    # Create nav bar with onclick evaluator
